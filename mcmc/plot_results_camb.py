...
path = 'output_camb3/mcmc'
save_path = 'saves/figures/camb/'
save_path_dat = 'saves/data/'

wanted_params = ['H0','ombh2','omch2','logA','ns','tau'] # parameters to plot

index = [7, 3, 4, 1, 2, 5] # index of the parameters in the txt file
...
names = ['weight', 'minuslogpost', 'cosmomc_theta', 'logA', 'ns',
         'ombh2', 'omch2', 'tau', 'As', 'H0',
         'minuslogprior', 'minuslogprior__0', 'chi2', 'chi2__so_forecast'
...
np.savetxt(save_path_dat+'camb_param.dat', mcmc_params)
np.savetxt(save_path_dat+'camb_sigma.dat', mcmc_sigma)
...
for i, name in enumerate(wanted_params):
    ...
    ax.plot(x, y_fish / np.max(y_fish), label = 'Fisher forecast',
            color = 'darkblue')
    #ax.plot(x, y_mcmc / np.max(y_mcmc), label = 'MCMC forecast', color = 'darkred')
    gdplot.add_1d(gd_sample, name, ax = ax, normalized = False, color = 'darkred', label = 'MCMC forecast camb', lw=2)
